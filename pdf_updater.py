...
DEFAULT_CHANGE_CONFIG = BASE_DIR / "changes.toml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Modifica los PDFs de input_pdfs usando un archivo TOML de cambios.",
    )
    parser.add_argument(
        "-c",
        "--config",
        default=os.getenv("PDF_UPDATER_CONFIG", str(DEFAULT_CHANGE_CONFIG)),
        help="Ruta al archivo de cambios TOML. Por defecto: changes.toml",
    )
    parser.add_argument(
        "--input-dir",
...
def generate_previews_for_inputs(pdf_paths: list[Path]) -> None:
...
def main() -> int:
    ...
    if generate_previews:
        generate_previews_for_inputs(pdf_paths)

    for pdf_path in pdf_paths:
        output_path = OUTPUT_DIR / pdf_path.name
        process_pdf(pdf_path, output_path, dry_run, change_config)
